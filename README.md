# 前言：
初衷：作为一个前端攻城狮，需要不断的积累总结和更新知识点，在这个过程中如果能对某个具体的知识点有系列的链接，不用再网上去筛选，而是有一个固定的地方可以查阅、搜索，那必然减少很多筛选的成本。在这里，好的链接被保存，经典的例子被保存，如果可以，进行一个某知识点排名。组件化开发的思想。代码规则那么多，终归是记不了那么多的，所以有快速定位、查找到相关知识点事十分高效的，这少不了平时的积累！
